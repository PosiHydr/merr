    ...
    ({\
        ...
        {\
            fprintf(\
              ...
              _MERR_INFO_ARGS_(estr, #name, #args)\
            );\
            _Exit(eret);\
        }\
    })

#define MERR_MODEL_1(init, jdge, estr, eret, retf, type, name, type1, args...)\
    ({\
        init;\
...
#define MERR_MODEL_VOID_1(init, jdge, estr, eret, name, type1, args...)\
    ({\
        init;\
        type1 _merr_arg1_ = _MERR_ARG1_(args);\
